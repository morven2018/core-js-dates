...
function getNextFriday(date) {
  ...
  const delta = weekday < 5 ? 5 - weekday : 7 - weekday + 5;
  const res = new Date(date.setDate(date.getDate() + delta));
  return res;
}
...
function isDateInPeriod(date, period) {
  ...
  return startDate <= initDate && endDate >= initDate;
}
...
function formatDate(date) {
  ...
  const minutes = pad(initDate.getUTCMinutes());
  const sec = pad(initDate.getUTCSeconds());

  return `${month}/${day}/${year}, ${hour || 12}:${minutes}:${sec} ${aPM}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const days = Math.floor((date - firstDayOfYear) / (1000 * 24 * 60 * 60));
...
function getNextFridayThe13th(date) {
  ...
  while (tempDate.getDate() !== 13 || tempDate.getDay() !== 5) {
    tempDate = getNextFriday(date);
  }
  return tempDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    ...
    if (work < countWorkDays) {
      workingDates.push(dateString(day));
      work += 1;
    } else if (off + 1 < countOffDays) off += 1;
    else {
      off = 0;
      work = 0;
    }
